...
import aso.mo.asoplayer.common.fadeInSlideInHorizontally
import aso.mo.asoplayer.common.fadeOutSlideOutHorizontally
import aso.mo.asoplayer.main.common.callbacks.TextWatcher
...
class SearchFragment : Fragment(), View.OnClickListener {
    ...
    private fun observeViewModel() {
        viewModel.songsResults.observe(
            viewLifecycleOwner,
            Observer { updateResultState(Type.Songs, it) })

        viewModel.albumsResults.observe(
            viewLifecycleOwner,
            Observer { updateResultState(Type.Albums, it) })

        viewModel.artistsResults.observe(
            viewLifecycleOwner,
            Observer { updateResultState(Type.Artists, it) })

        viewModel.genresResults.observe(
            viewLifecycleOwner,
            Observer { updateResultState(Type.Genres, it) })

        viewModel.playlistResults.observe(
            viewLifecycleOwner,
            Observer { updateResultState(Type.Playlists, it) })
        ...
        viewModel.resultSize.observe(viewLifecycleOwner, Observer {
            ...
            filterAnimator = if (it == 0) {
                ...
                if (filterButton.visibility == View.VISIBLE) filterButton.fadeOutSlideOutHorizontally(
                    duration = 500
                ) else null
                ...
                if (filterButton.visibility != View.VISIBLE) filterButton.fadeInSlideInHorizontally(
                    duration = 500
                ) else null
            }
    ...
    private val searchTextWatcher = object : TextWatcher {
        override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
            ...
            filterAnimator?.cancel()
            filterAnimator =
                filterButton.fadeOutSlideOutHorizontally(duration = 500).apply { start() }
