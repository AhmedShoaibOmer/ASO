...
import aso.mo.asoplayer.common.crossFadeWidth
...
import aso.mo.asoplayer.main.common.view.BaseFullscreenDialogFragment
import kotlinx.android.synthetic.main.fragment_add_songs_to_playlists.*

class AddSongsToPlaylistsFragment : BaseFullscreenDialogFragment(), OnItemClickListener,
    ...
    private fun updateViews(data: Any) {
        ...
        if (data is Event<*>) {
            data.getContentIfNotHandled()?.let {
                ...
                crossFadeAnimatorSet =
                    doneButton.crossFadeWidth(progressBar, 600, visibility = View.INVISIBLE)
                if (it.message != null) Toast.makeText(activity, it.message, Toast.LENGTH_SHORT)
                    .show()
    ...
    private fun updateSelectedCount() {
        ...
        if (count == 0) {
            crossFadeAnimatorSet?.cancel()
            crossFadeAnimatorSet =
                unselectedButtons.crossFadeWidth(doneButton, 600, visibility = View.INVISIBLE)
        } else {
    ...
    override fun onClick(v: View?) {
        when (v?.id) {
            ...
            R.id.addPlayListIcon -> findNavController().navigate(
                AddSongsToPlaylistsFragmentDirections.actionAddSongsToPlaylistsFragmentToWritePlaylistDialogFragment()
            )
        }
    }
