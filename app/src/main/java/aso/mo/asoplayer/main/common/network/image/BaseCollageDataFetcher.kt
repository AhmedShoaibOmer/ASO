...
import aso.mo.asoplayer.common.inputStream
...
abstract class BaseCollageDataFetcher(
    ...
    override fun getDataSource(): DataSource =
        if (useFile()) DataSource.LOCAL else DataSource.REMOTE
    ...
    private fun getImageUrl(album: Album): String? {
        StrictMode.setThreadPolicy(
            StrictMode.ThreadPolicy.Builder().detectAll().permitNetwork().build()
        )
        var params = mapOf(
