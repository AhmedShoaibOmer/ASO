...
import aso.mo.asoplayer.main.common.callbacks.MediaItemDataDiffCallback
...
class CurrentSongsFragment : BaseFragment(), OnItemClickListener {
    ...
    private fun setupViewViews() {
        ...
        val adapter =
            BaseAdapter(items, activity!!, R.layout.item_current, BR.mediaItem, this, null)
    ...
    private fun observeViewModel() {
        viewModel.mediaItems.observe(viewLifecycleOwner, Observer {
            (currentRV.adapter as BaseAdapter<MediaItemData>).updateItems(
                it,
                MediaItemDataDiffCallback(items, it)
            )
            items = it
        })
