...
class PlaylistSongsEditorViewModel(application: Application) : SongsViewModel(application) {
    ...
    override fun init(vararg params: Any?) {
        playlistSongsUri =
            MediaStore.Audio.Playlists.Members.getContentUri("external", params[0] as Long)
        super.init()
    }
    ...
    override fun deliverResult(items: List<Song>) {
        ...
        viewModelScope.launch {
            withContext(Dispatchers.IO) {
                val songsInPlaylist =
                    playlistSongsRepository.loadData(playlistSongsUri, playlistSongsProjection)
                items.forEach { song ->
    ...
    private fun deleteSongs(songs: List<Song>): Boolean {
        ...
        val deletedRows =
            getApplication<Application>().contentResolver?.delete(playlistSongsUri, where, null)
                ?: 0
    ...
    private fun addSongs(songs: List<Song>): Boolean {
        ...
        songs.forEachIndexed { index, song ->
            ...
            value.put(
                ...
                initiallySelectedItems.size + 1
            )
            value.put(MediaStore.Audio.Playlists.Members.AUDIO_ID, song.id)
        }
        val addedRows = getApplication<Application>().contentResolver?.bulkInsert(
            playlistSongsUri,
            contentValues
        ) ?: 0
        return addedRows > 0
